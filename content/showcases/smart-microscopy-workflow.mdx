...
description: A no-code Smart Microscopy workflow that performs 3D live monitoring of cell clusters by combining segmentation, clustering and adaptive acquisition.
group: paper
tags: ["Smart microscopy", "Real-time", "No-code"]
version: paper
---

import { ShowcasePrerequisites } from "@/components/docs";
...
![The Smart Microscopy Workflow](/docs/showcases/paper/smart.png)
...
to the microscope to acquire all clusters with a 40X objective. To install this workflow [connect this website](/docs/introduction/first-steps/brief-interlude#we-have-an-app-for-that) to your Arkitekt instance and click on the Import button below:

<DisplayWorkflow />

A few things to note about this workflow:

1. This workflow utilizes a pretrained Stardist model to segment the nuclei in the image. This model comes preinstalled with the Segmentor plugin, by changing this action
   with a `Predict Stardist` action you can use your own Stardist model.
...
You do not need to run this workflow directly, as it will be called by the `Multi Dimensional` workflow.
...
<div className="mt-2 mb-2 text-sm text-fd-muted-foreground">
  <small>
    Once connected, green nodes are installed and yellow ones still lack an implementation.
  </small>
</div>

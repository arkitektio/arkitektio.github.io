...
description: A streaming Arkitekt workflow that performs real-time StarDist segmentation on 3D microscopy data as it is being acquired.
group: paper
tags: ["Streaming", "StarDist", "Real-time"]
version: paper
---

import { DownloadGrid, ShowcasePrerequisites } from "@/components/docs";
import { DisplayWorkflow } from "@/components/arkitekt";
import { InstallRepoButton } from "@/components/arkitekt";
import { UploadModelButton } from "@/components/arkitekt";

Classically, images are batch-processed once the acquisition is complete, leaving no chance for real-time feedback on a running experiment; why that matters is laid out in
[Real-time](/docs/design/philosophy/realtime) and practised in the [Live Analysis](/docs/introduction/advanced/live) tutorial. In this second workflow, we illustrate how Arkitekt with its ability for streaming analysis (i.e., processing immediately after each acquisition event), enables to display real-time feedback on the running acquisition.
...
![The Streaming Microscopy Workflow](/docs/showcases/paper/streaming.png)
...
<ShowcasePrerequisites />

Additionally, we suggest to go through the [Live Analysis Basics](/docs/introduction/advanced/live) guide to get familiar with the Streaming
capabilities of Arkitekt.

### Hardware Requirements

The analysis computer in the illustrated example runs Ubuntu 22.04, but the workflow was equally tested on a Windows 10 machine.
...
This workflow uses the Gucker App to watch a directory for new files on the Acquisition Computer. Installation and configuration are described on the
[Gucker page](/docs/apps/standalones/gucker).
...
the mikro-napari plugin by following the instructions [here](/docs/introduction/first-steps/first-app) or [here](/docs/apps/standalones/mikro-napari).

#### PluginApps

This workflow uses the following `PluginApps` installed through the Arkitekt Plugin Store. To install these plugins follow the instructions [here](/docs/introduction/first-steps/first-tool),
or [connect this website](/docs/introduction/first-steps/brief-interlude#we-have-an-app-for-that) to your Arkitekt instance and click on the button below:
...
2. Start Gucker and set it up as described in [Gucker: Configuration](/docs/apps/standalones/gucker#configuration). For this example the "Import" directory is `C://Data`
   from the previous step; the "Export" directory can be any other writable directory.

3. Connect and Login with the Arkitekt instance (if your Arkitekt instance does not show up, connect it through the [mesh](/docs/design/security/mesh) or enter its URL manually)
...
<video
  ...
  width={"100%"}
>
  <source src="/docs/showcases/paper/organoid_workflow_creation.webm" />
</video>
...
1. During the creation of the workflow around (2:25) we see the `Predict Stardist` node being configured. Here we can see that the `Segmentation Model` parameter is set to be a global parameter, and that
   we are manually pointing the node at all three segmentor instances that we started in the previous step. This will send out incoming requests in a round robin manner to the three
...
<Callout type="info" title="On Parallization in Arkitekt">
...
<Callout type="info" title="On Stages">
...
Open the deployed workflow either on the Arkitekt Workflow interface or on the Dashboard, and once all of the Applications are started, Arkitekt should guide you
...
2. RegExp: This is a regular expression that will be used to filter the files that are found in the "Import Directory". In our case we want to only segment the images
   that are saved in the TIF format, so we used the regular expression `*.TIF` to only select these files. More patterns are listed in
   [Gucker: Usage in Arkitekt Workflows](/docs/apps/standalones/gucker#usage-in-arkitekt-workflows).

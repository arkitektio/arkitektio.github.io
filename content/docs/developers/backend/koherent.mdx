...
provenance is and how a single task groups many changes.
...
It does **not** authenticate requests, mint tokens, or define the transport; how those jobs are
split between `authentikate`, `kante` and Koherent is laid out in
[Backend Libraries](/docs/developers/backend). Koherent is the piece that turns an authenticated,
context-carrying request into a durable, queryable audit trail.

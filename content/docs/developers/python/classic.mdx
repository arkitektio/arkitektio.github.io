...
description: An introduction to the classical Arkitekt app style, the App context manager, and how it connects your code to the Arkitekt server.
---


Arkitekt's client SDK is designed to be used in a declarative and functional way, with a focus on reusable functions
and composability: most functionality is provided through functions rather than classes, so it fits the patterns of
the scientific Python developer. The one stateful exception is the Arkitekt `App`, which encapsulates everything that
needs managing (authentication, waiting for assignments from the server, ...). Let's create one, as it is the connection
layer between your code and the Arkitekt server. If you don't know what we mean by an App, check out the
[App](/docs/design/terminology/app) section of the terminology page.
...
1. You might notice that we are creating an arkitekt app through calling the `easy` builder function, specifying only the name of the app, without having to explicitly
   ...
   On the first connection a browser opens and you approve the device login, just as you did in [Almost there](/docs/introduction/first-steps/interface#almost-there).
   ...
   ```python
    ...
    app.fakts.grant = AnyOtherGrantType()
    with app:
        # do stuff with arkitekt
   ```
 Depending on your environment you can also use other builders, like the `jupy` builder for use inside a Jupyter notebook.
...
3. Within this context, you can use the `app` object to access the various services that
   ...
   For example, to access the `Mikro` client (responsible for storing your microscopy data
...
- [Storing image data](/docs/developers/python/read-write)

...
description: Troubleshooting connected apps and plugins, covering repositories, appifying, and plugin installation.
---
...
The install steps themselves (Add repository, Appify, Deploy) are walked through in
[Installing our first App](/docs/introduction/first-steps/first-tool#installing-our-first-app). Where did you get stuck? We will guide you through a few checkpoints.
...
also make sure that the repository is public. Currently we do not support private repositories. Use the repository's default branch in the `Branch` field:
`main` for newer repos and `master` for older ones (e.g. `arkitektio-apps/stdlib` uses `main`, `arkitektio-apps/reaktor` and `arkitektio-apps/omero` use `master`).

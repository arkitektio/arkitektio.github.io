...
description: Step-by-step troubleshooting for installing Docker, running Konstruktor, and getting your Arkitekt deployment up and running.
---

Puh. This is probably the worst case scenario, and one of the hardest to debug. But don't worry, we will get you through this.
Start with the [general tips](/docs/troubleshooting/general) if you haven't.

## Before we start

This troubleshooting section will assume you have read the [Getting Started](/docs/introduction/first-steps) section and have a basic understanding of how Arkitekt works.
...
<Callout type="info" title="Please open an Issue on GitHub">
...
2. Restart Discovery: restart the whole Orkestrator app.

3. Make sure you can reach your deployment, either on the same machine, or across networks through the [mesh](/docs/design/security/mesh). (Local-network advertising has been removed, see the [installation guide](/docs/introduction/installation/starting-new#advertising).)

4. For automatic detection: Make sure that:

   - Your deployment and the Desktop App have both [joined the mesh](/docs/introduction/installation/starting-new#mesh), so they can discover each other.
...
5. To reach a deployment across networks (a different building, NAT, or off-site), use the [mesh](/docs/design/security/mesh) rather than relying on a shared local network.

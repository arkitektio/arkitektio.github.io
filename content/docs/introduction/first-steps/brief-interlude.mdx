...
description: A reflection on Arkitekt's separation of concerns and how this documentation website itself acts as an app to install plugins and workflows.
---

import { Guard } from "@/components/arkitekt";
import { InstallRepoButton } from "@/components/arkitekt";

We now have a basic understanding of how to call an conversion action on your data, we can visualize the results, now before we continue, lets address a few things.
...
- **Reusability**: The concept of Actions might seem alien at the start, but it allows for a more worry free design of your workflows. You can easily use actions from different apps
  in your workflow, and not worry about how they are implemented.

- **Customizability**: The concept of Actions and Plugins allows you to easily extend the functionality of your Arkitekt instance, without having to worry about breaking anything. Arkitekt
...
<div className="bg-back-800 p-3 text-center rounded-sm rounded-md mb-2">
 😊 Just one connection away... 
</div>

<Callout type="warn" title="Not able to connect?">
...
<div className="bg-green-300 text-center text-black p-3 rounded-sm rounded-md ">
...
Finally you will be able to do some proper image analysis work, this is the Arkitekt Standard Library (`stdlib`), which comes with an ever increasing number of actions that you can use
to analyze your data. They are not as fancy as the ones we have seen so far, but they are very useful. The button below adds the repository to your Arkitekt instance; then
install it like before (`Appify` the latest version and `Deploy` it, see [Installing our first App](/docs/introduction/first-steps/first-tool#installing-our-first-app)).

<Guard fallback={<Callout type="warn" title="Not connected yet">Connect this website to your Arkitekt instance (see above) and the install button appears here.</Callout>}>

<InstallRepoButton repo={"stdlib"} user={"arkitektio-apps"} branch={"main"} />

</Guard>

Turned green? Great! You now have the `stdlib` repository installed. We will use it in the next chapter.

#### Second plugin: `reaktor`

The second plugin we will install is called `reaktor`. This is a weird one, because it is not really a plugin that does anything. It is a plugin that will allow us to run
(schedule) your workflows directly on the Arkitekt server. You might not understand what that means right now, but you will later. Same procedure as before: click the button, then appify and deploy.
...
<Callout type="info" title="We love this feature">

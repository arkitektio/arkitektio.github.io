...
description: Convert your first file with an action, inspect the result, and mark a ROI to understand the Arkitekt Data Graph.
---
...
- Inspect the result in Orkestrator
- Mark a ROI (Region of Interest) in our Image

## Convert our first File

We will start by converting our first file, utilizing our "Convert Omero" action. Lets perform this conversion on our
...
<video
  ...
  width={"100%"}
>
  <source src="/docs/introduction/first_steps/convert.webm" />
</video>
...
<Callout type="warn" title="TODO">

Clarify if the user needs to ensure the "Omero" app is connected/running for this option to appear, or if it appears even if the app is offline (and would trigger a start).
...
converted zarr dataset again to the server. More on that in [How Tasks Work](/docs/design/terminology/task). You will notice that
...
<video
  ...
  width={"100%"}
>
  <source src="/docs/introduction/first_steps/visualize.webm" />
</video>
...
<Callout type="info" title="Visualization within Orkestrator">
...
<Callout type="info" title="On Mikro next">
...
metadata in a more flexible way. This will also allow us to provide more advanced visualization options in the future..
...
<video
  ...
  width={"100%"}
>
  <source src="/docs/introduction/first_steps/roi_mark.webm" />
</video>

A few words about the `Data Graph` in Arkitekt: it is the graph of relationships between data elements and their metadata, here the one kept by the `Mikro` service
that stores our microscopy data. We explain it properly, and how it differs from the provenance graph, in [Your first Workflow Run](/docs/introduction/first-steps/first-run).
For now it lives from interlinking data points, and we have already created a few: our original file, our converted image and our ROI, linked in the following way
...
<Callout type="info" title="A bit technical">

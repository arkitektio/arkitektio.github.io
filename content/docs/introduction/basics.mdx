...
Arkitekt is software platform for image analysis. As explained in the [philosophy](/docs/design/philosophy) section, Arkitekt wants to act as
a middleman and datahub that stands between the user and the image analysis tools, the central hub they all connect to. As such, Arkitekt is an installable app that runs on a computer in your
...
Anyway, when you install Arkitekt, you are installing that server app. It will now run on your computer, in the background, ready for Apps to connect to it (on the same machine, or across networks through the [mesh](/docs/design/security/mesh)).
We will talk more about Apps later on, but for now just think of them as the image analysis tools that you want to use.

...
Inherently when you build a workflow in Arkitekt you _always_ build a streaming analysis workflow. Just a very simple one, where the real-time functionality does not matter. In a basic workflow
...
As discussed in the first steps tutorial, Arkitekt is build around the concept of actions, that define a procedure that can happen on a specific item, and are defined by their input and outputs.
...
and which is an action that can take an item and returns (yields) item(s) _multiple times_ and importantly **over time**

![Functional Actions vs Stream Actions](/docs/introduction/advanced/node-stream.png)

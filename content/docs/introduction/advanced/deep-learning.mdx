...
- Have a CUDA capable GPU. If you don't have a GPU, you should still be able to follow along, but the prediction
  will be very slow.
- Either Mikro-Napari or Fiji with the [ImageJ Arkitekt plugin](/docs/apps/standalones/imagej-plugin) installed. If you don't have either of these, you can still follow along, but you won't be able to run the workflow.
...
![Nuclei segmentation](/docs/introduction/advanced/segmentation.png)
...
- How to maintain the **complex software requirements** that is needed for deep learning.
  ...
  (see [Why do we go through this hurdles](/docs/introduction/first-steps/first-tool#why-do-we-go-through-this-hurdles)).
...
Now that we have tested our plugin, we would like to integrate it with our favourite bioimage analysis app. In this case we will be using Fiji through the ImageJ Arkitekt plugin, but you can use any other app that is supported by Arkitekt.
...
Note that in current versions a saved workflow is runnable right away, see [Your first Workflow Run](/docs/introduction/first-steps/first-run) for how runs are started now.
...
<div className="mt-2 mb-2 text-sm text-fd-muted-foreground">
  <small> The workflow in action utilizing Fiji</small>
</div>

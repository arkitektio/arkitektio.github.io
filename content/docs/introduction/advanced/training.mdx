...
description: How to train and use your own deep learning model in Arkitekt with CARE, using Contexts to relate datasets for image restoration.
---
...
first steps of the [Arkitekt tutorial](/docs/introduction/first-steps/interface) and the [Deep Learning Bridge tutorial](/docs/introduction/advanced/deep-learning).
...
we have a very simple conceptional understanding of training, which is that training a deep learning model is simply an `Action` that returns a `Model` that in a second step can be used to
make predictions (in a `Prediction Action`). This means that training a model is as simple as running an `Action` that returns a `Model`. The developer can then decide how to train the model, and to save it in
...
<video
  ...
  width={"100%"}
>
  <source src="/docs/introduction/advanced/upload_train_data.webm" />
</video>
<div className="text-gray-400 mb-2">
  <small> Uploading an converting the data</small>
</div>

This steps should seem [familiar](/docs/introduction/first-steps/interface), just a few notes:

- We are batch uploading the data by selecting both files and `multi-dropping` them on the webinterface
- Then we are batch converting the data through the `Convert Omero` action, which will convert the data to the `Image` datatype.
- We now have two `Image` datatypes in our project, which we can see in the `Data` tab.

### Creating a Context

Now that we have our data, we can create a `Context` that relates the two `Image` datatypes. To do this we could either use the `Create Context` button, or have an `Action` that returns a `Context` datatype, and
...
<video
  ...
  width={"100%"}
>
  <source src="/docs/introduction/advanced/create_context.webm" />
</video>
<div className="text-gray-400 mb-2">
...
Make sure to appify and deploy the plugin, as shown in [Installing our first App](/docs/introduction/first-steps/first-tool#installing-our-first-app).

This app provides only two actions `Predict CARE` and `Train CARE`, which allow you to train and predict CARE models, with your data. The Train CARE action will take a `Context` as input, and will return a `Model` as output.
and the `Predict CARE` action will take a `Model` and an `Image` as input and will return a `Predicted Image` as output. Actions all the way down. Lets see how we can train a CARE model.
...
<video
  ...
  width={"100%"}
>
  <source src="/docs/introduction/advanced/train_care.webm" />
</video>
<div className="text-gray-400 mb-2">
  <small> Training a CARE Model</small>
</div>

A few notes:

- You know the drill. Search for the `Train CARE` action and select it to make it available.
...
- Once you submit the dialog, the training will start. You can see the progress in the `Dashboard` tab, and you will see the `Train CARE` action turning green once the training is done.
...
<video
  ...
  width={"100%"}
>
  <source src="/docs/introduction/advanced/inspect_container.webm" />
</video>
<div className="text-gray-400 mb-2">
...
For this we will now use the `Predict CARE` action, which in this case will take a `Model` and an `Image` as input and return an image.
...
Also of course you can download the model and share it with others, just like we did with the models that we are using in our [Showcases](/showcase).
...
But also please just find here **ALL** the code for the `Train CARE` and `Predict CARE` actions, which are just a few lines of code, and should be easy to understand.

...
protocol version" apart from "server unreachable". The differences are laid out
in [What's changed: Connecting apps](/docs/design/whats-changed/connecting).
</Callout>
...
The Paper version used a custom `start`, `challenge` and `claim` sequence and a separate OAuth2
client (Herre) for tokens. Every step of that sequence has a counterpart in the flow above, and the

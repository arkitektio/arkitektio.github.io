---
title: Deployment
description: A high-level overview of Arkitekt's customizable deployment strategies and the considerations behind them.
---
...
<Callout type="warn" title="On the production readiness of Arkitekt">
...
<Callout type="info" title="On Plugin Apps and GPU Support">
...
Kabinet service, we are planning to provide better support for GPU sharing. If you are interested in running multiple GPU intensive Plugin Apps
on Arkitekt, please make sure that you have enough GPUs available. How GPUs are made available to containers on each operating system is
described in [GPU Support](/docs/design/various/gpu).
...
Arkitekt was built with the idea in mind that it should be highly customizable in its deployment strategies. This is why
we have built Arkitekt in a way that it can be used to maintain Arkitekt from within a python process ([development](/docs/design/deployment/testing)),
or as a local development setup (through the CLI), or via a GUI installer. The CLI and the GUI installer are both front ends of
Konstruktor, and are described in the [installation guide](/docs/introduction/installation/starting-new), together with the
[other installation strategies](/docs/introduction/installation/starting-new#other-installation-strategies) such as a maintained
Docker Compose configuration and Helm charts.

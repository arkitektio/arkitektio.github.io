...
description: How Arkitekt enables real-time, streaming bioimage analysis instead of slow post-hoc batch processing.
---
...
Arkitekt is built to enable users to design analysis workflows that can as **easily run post-hoc\*** on already generated data, as they can run in **real-time on data** while it is being generated. This requires a bit of a different approach to designing and thinking about bioimage workflows: Instead of focussing on "batch analysis", Arkitekt fully embraces "streaming analysis" to analyses data as it is being generated, no only cutting down on the time it takes to get results, but because of immediate feedback enabling paradigms like "smart microscopy". The technical side of this argument, stateful apps that load a model once instead of once per image, is laid out in the [Nextflow comparison](/docs/design/comparisons/nextflow#execution-model).

<div className="mt-2">

![Streaming analysis](/docs/design/streaming.png)

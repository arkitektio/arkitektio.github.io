---
title: Manifest
description: A manifest is the Arkitekt way of identifying an application, describing its name, version, and required access scopes.
---
...
When an app connects to a deployment through the
[Fakts protocol](/docs/design/architecture/service-discovery), the manifest also
declares which services the app needs. Each requirement names a `key` the app
uses to look the service up, the `service` identifier it must be an instance of,
...
The user sees these requirements when approving the app and picks, for each one,
which running instance of that service the app is allowed to use. A required
service that is missing or denied stops the app from connecting, an optional one
simply switches that feature off.
...
available in the `stable` channel. It is currently only available in the `beta` and `next`
channels (see [How we version Arkitekt](/docs/design/philosophy/version)). It is however already part of the CLI.
...
Flavours and Selectors are a way of describing the different ways an app can be deployed (for example a CUDA build and a CPU-only build of the same app). How the platform picks a flavour at deploy time is described in [App Flavours](/docs/design/services/kabinet#app-flavours).
...
`stable` and `beta` channels, and the api is subject to change. This feature is currently only
available in the `next` channel (see [How we version Arkitekt](/docs/design/philosophy/version)).

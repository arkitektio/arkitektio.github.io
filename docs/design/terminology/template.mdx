...
the inputs, outputs, but **not** the actual code that is run. A template is a concrete implementation of a node, and implements the abstract
notion of the task in a specific application.
...
template for each bioimage task based on the user's preferences and the available resources.

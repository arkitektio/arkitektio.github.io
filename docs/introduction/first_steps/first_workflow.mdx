...
-   About the `Nodes`.
    ...
    notice the node termed `Input` and `Output`. These are special nodes that are part of every workflow. They are the entry and exit points to your analysis.
...
-   About their colors:    
    ...
    If you have connected the website and followed the tutorial just until now, you might notice that the color of the nodes is yellow. This is because
...
- The second type are `partially distributed` Schedulers.
  ...
  `partially distributed` or `ìn-app` Schedulers are Schedulers that will call their own nodes directly, *without* the need to call Arkitekt. This means that they can schedule tasks much faster, as they don't have to call Arkitekt for each step. 
...
4. Set necessary parameters on the sidebar.
    ...
    by the `Measure Fraction` node. You can do that by clicking on the node, and then setting the parameters in the sidebar. We are interested in the `Fraction` of the image that is above the threshold,
    so we can leave it at the default value ("1", As all values above the threshold will be set to 1).

5. Save the workflow

    You can save the workflow by clicking on the "Save" button on the bottom right. This will save the workflow to your server, mkaing it ready to be deployed.

...
- The second type are `partially distributed` Schedulers.
  ...
  You can see an example of that in our experimental [MikroJ in memory workflow](/docs/showcases). The downside is that they are more fault-prone, as they are not running on the Arkitekt server
...
in other parts of the [documentation](/docs/design/terminology/scheduler).
:::

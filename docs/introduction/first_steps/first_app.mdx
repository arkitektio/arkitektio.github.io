---
title: Napari and Fiji
sidebar_label: 5. Bridging Napari and Fiji
sidebar_position: 5
---
...
with Arkitekt and together in your workflows. Of course feel free to skip the section that is not relevant to you.
...
- Install it directly from the command line:
  ...
  If you are familiar with command line and you have napari installed in a python environment, you can also install the plugin directly from the command line.
...
1. **The Magic Bar**:
    ...
    you will encounter it in almost all apps that currently connect to Arkitekt.
2. **The Connection screen**:
    ...
    For remote instances, [advertised](docs/introduction/installation#advertisting) Instances will be automatically detected and should be listed in the dropdown menu. If 
...
Once you have connected to your Arkitekt instance, your Napari instance should now both appear on the Dashboard and the provide button should now be blinking red (red means
...
2. *ROIs*:
    ...
    assigning the node. Importantly you will notice that if you mark rois on the ROI layer they are *live syncing* with your server and the browser, and
...
will interface with your Fiji installation and allows us to connect Fiji to Arkitekt. 

1.  Just download MikroJ from our [Github page](https://github.com/jhnnsrs/mikroj) and extract it to a folder of your choice. 
    Then double click on the "MikroJ.exe" file to start the app. Please do *NOT* start your FIJI installation. MikroJ will start Fiji for you.
...
3.  Click on Connect ImageJ. This will open a dialog and prompt you to navigate to your "Fiji.App" folder that contains your ImageJ installation.
    ...
    However this step might take a few seconds and cause an UI freeze. This is normal, and should resolve in a few seconds to a minute, while MikroJ
    and maven (the Java packaging tool that we use) are resolving some dependencies.

    :::

4. Optional: Create a shortcut to MikroJ on your desktop. This will allow you to start MikroJ directly from the shortcut.

If nothing happens after you click on "Connect ImageJ", please refer to the [troubleshooting section](/docs/troubleshooting/app).
...
1. *Cross Talk*:
    When calling the "Load Into Imagej" node, MikroJ will download the whole image into memory and share this image with Fiji. This means that you can
...
2. *ROIs*:
    ...
    yet found a good solution. If you have any ideas, please reach out.
...
3. *Alterations*:
    ...
    use the "Upload active Image" node. More on that later.

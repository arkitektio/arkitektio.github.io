...
And then you can install the latest version of segmentor by deploying it as an plugin, as you learned in the [basics of Arkitekt](/docs/introduction/first_steps/first_tool#installing-our-first-app).
...
Just import this workflow into your Arkitekt instance, and deploy it on the `reaktor` scheduler just like you did in the [basics of Arkitekt](/docs/introduction/first_workflows#reserving-a-scheduler).
...
Here we have used a pretrained model, but what if we want to use our own model? Well that's easy. Training your own model on the segmentor plugin is also quite easy, but lets explore that in the [next tutorial](/docs/introduction/advanced_tutorial/training)
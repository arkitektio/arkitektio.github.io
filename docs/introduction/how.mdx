...
Good question. Arkitekt is software platform for image analysis. As explained in the [philosophy](/docs/philosophy) section, Arkitekt wants to act as 
...
lab (or just yours if you don't want to share). In more programmatic terms this app is in actually server platform, that provides a graphical webinterface for the user to interact with, and also some
...
Anyway, when you install Arkitekt, you are installing that server app. It will now run on your computer, in the background, and advertise itself on your local network, so that
...
These webservices and maintained in docker containers and orchestrated by docker-compose (at least in the default one computer setup). All webservices follow
an API First Design and expose a comprehensive GraphQL API (More on GraphQL in its ([section](/design/api)). All webservices are able to work standalone. The main webservices are:
...
decentralized authentication and authorization for all Arkitekt services through signed json web tokens. Lok also provides the endpoints for the fakts API 
